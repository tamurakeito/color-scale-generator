...
function getGrayscaleColor(hex) {
    let [r, g, b] = hexToRgb(hex);
    let gray = Math.round(0.299 * r + 0.587 * g + 0.114 * b);
    return rgbToHex(gray, gray, gray);
}
function getClosestGrayLevel(grayHex) {
...
function generateColorScale(centerLevel, centerColor) {
    const minColor = [255, 255, 255]; // 000 = #FFFFFF
    const maxColor = [0, 0, 0]; // 1000 = #000000
    ...
    for (let i = 1; i < scaleLevels.length - 1; i++) {
        ...
        let interpolatedRGB = interpolateColor(i < centerIndex ? minColor : centerRGB, // 明るい側は白から補間
        i > centerIndex ? maxColor : centerRGB, // 暗い側は黒から補間
        factor);
        result[scaleLevels[i]] = rgbToHex(interpolatedRGB[0], interpolatedRGB[1], interpolatedRGB[2]);
    }
    return result;
}
// 入力処理
if (typeof process !== "undefined" && process.stdin) {
    ...
    rl.question("HEXカラーコードを入力してください（例: #036A8B）: ", (userInput) => {
